...
<body>
    ...
    <h2>Items</h2>
    <table><thead><tr><th>Product</th><th>Size</th><th>Quantity</th><th class="right">Amount</th></tr></thead><tbody>@foreach($invoiceItems as $item) @php($invoiceProduct = $invoiceProducts->get($item->product_id))<tr><td>{{ $invoiceProduct->name ?? 'Product' }}@if($invoiceProduct && $invoiceProduct->color)<br><span class="muted">Color: {{ $invoiceProduct->color }}</span>@endif</td><td>{{ $item->variation_name ?: 'Standard' }}</td><td>{{ $item->qty }}</td><td class="right">{{ number_format($item->subtotal, 2) }}</td></tr>@endforeach</tbody><tfoot><tr><td colspan="3" class="right">Subtotal</td><td class="right">{{ number_format($getOrderDetail->subtotal, 2) }}</td></tr><tr><td colspan="3" class="right">Shipping</td><td class="right">{{ number_format($getOrderDetail->shipping_amt, 2) }}</td></tr><tr><td colspan="3" class="right">Tax</td><td class="right">{{ number_format($getOrderDetail->tax_amt, 2) }}</td></tr><tr><td colspan="3" class="right"><strong>Total</strong></td><td class="right"><strong>{{ number_format($getOrderDetail->net_amt, 2) }}</strong></td></tr></tfoot></table>
