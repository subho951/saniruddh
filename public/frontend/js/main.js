(function ($) {
    ...
    $('.add').on('click', function () {
        ...
        var maximum = parseInt($quantity.attr('max'), 10);
        if ($quantity.val()) {
            var nextValue = +$quantity.val() + 1;
            $quantity.val(Number.isNaN(maximum) ? nextValue : Math.min(nextValue, maximum));
        }
    });
    $('.sub').on('click', function () {
        var $quantity = $(this).next();
        var minimum = parseInt($quantity.attr('min'), 10) || 1;
